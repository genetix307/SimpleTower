function reset_progress(){

//MISC
store.game_speed=1.00
store.max_gamespeed=2.00
store.ads_watched=0
store.gem_chest_cooldown=0
store.show_rating=0
store.gems=0
store.version="version 1.00"
store.devMode=0
store.hold_dev=0

//Game Settings
store.color_scheme=4
store.sfx_volume=1
store.bgm_volume=1
store.bonus_discord=0
//store.auto_perk=0
//store.show_healthbars=0
//store.auto_upgrade=0
//store.auto_event=0
//store.story_scene=0
//store.tutorial=0

//Challenges
store.challenge_a=0
store.challenge_b=2
store.challenge_c=5
store.challenge_flag=0
store.goal_template=0
store.lvl_template=1

//Perm Stats 
store.perm_attack_speed=1
store.perm_attack_damage=3
store.perm_critical_chance=0
store.perm_critical_multiplier=1.30
store.perm_attack_range=150
store.perm_multishot_chance=0
store.perm_maxhp=10
store.perm_regen=0
store.perm_lifesteal_chance=0
store.perm_lifesteal_amount=1
store.perm_armor=0
store.perm_block_chance=0
store.perm_goldperwave=0
store.perm_goldperkill=1
store.perm_gemsperwave=0
store.perm_free_attack_upgrade=0
store.perm_free_defense_upgrade=0
store.perm_free_utility_upgrade=0

//Perm Upgrades
store.cost_perm_attack_speed=5
store.lvl_perm_attack_speed=1
store.cost_perm_attack_damage=5
store.lvl_perm_attack_damage=1
store.cost_perm_critical_chance=5
store.lvl_perm_critical_chance=1
store.cost_perm_critical_multiplier=5
store.lvl_perm_critical_multiplier=1
store.cost_perm_attack_range=5
store.lvl_perm_attack_range=1
store.cost_perm_multishot_chance=5
store.lvl_perm_multishot_chance=1
store.cost_perm_maxhp=5
store.lvl_perm_maxhp=1
store.cost_perm_regen=5
store.lvl_perm_regen=1
store.cost_perm_lifesteal_chance=5
store.lvl_perm_lifesteal_chance=1
store.cost_perm_lifesteal_amount=5
store.lvl_perm_lifesteal_amount=1
store.cost_perm_armor=5
store.lvl_perm_armor=1
store.cost_perm_blockchance=5
store.lvl_perm_blockchance=1
store.cost_perm_goldperwave=5
store.lvl_perm_goldperwave=1
store.cost_perm_goldperkill=5
store.lvl_perm_goldperkill=1
store.cost_perm_gemsperwave=5
store.lvl_perm_gemsperwave=1
store.cost_perm_free_attack_upgrade=5
store.lvl_perm_free_attack_upgrade=1
store.cost_perm_free_defense_upgrade=5
store.lvl_perm_free_defense_upgrade=1
store.cost_perm_free_utility_upgrade=5
store.lvl_perm_free_utility_upgrade=1

//Other Permanent Stats
store.cost_perm_gamespeed=5
store.lvl_perm_gamespeed=1

//Inventory
//store.have_template=1 //ex: have_potions

//cards 
/*
store.slot_2_unlocked = 0
store.slot_3_unlocked = 0
store.slot_4_unlocked = 0
store.card_cost = 10
store.card_slot_1 = 0
store.card_slot_2 = 0
store.card_slot_3 = 0
store.card_slot_4 = 0
store.card_inheritance = 0
store.card_lvl_inheritance = 1
store.card_heartpiece = 0
store.card_lvl_heartpiece = 1
store.card_mineshaft = 0
store.card_lvl_mineshaft = 1
store.card_orb = 0
store.card_lvl_orb = 1
store.card_critgold = 0
store.card_lvl_critgold = 1
store.card_rage = 0
store.card_lvl_rage = 1
store.card_supplydrop = 0
store.card_lvl_supplydrop = 1
store.card_axes = 0
store.card_lvl_axes = 1
store.card_evilcreed=0
store.card_lvl_evilcreed=1
store.card_mastery=0
store.card_lvl_mastery=1
*/
//Statistics
//store.best_stage=1
store.total_runs=0
store.enemies_slain=0
store.bosses_slain=0
store.gems_earned=0
store.gold_earned=0
//store.total_upgrades=0
store.challenges_completed=0

//Bonus Codes
store.code_payday=0
store.code_kickstart=0
store.code_lotto=0
}