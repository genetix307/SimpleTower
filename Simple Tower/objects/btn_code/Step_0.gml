if myResult = "payday" and store.code_payday = 0
{
reward = 200
store.code_payday = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "kickstart" and store.code_kickstart = 0
{
reward = 300
store.code_kickstart = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "lotto" and store.code_lotto = 0
{
reward = ceil(random(999))
store.code_lotto = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "fivestars" and store.code_fivestars = 0
{
reward = 300
store.code_fivestars = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "tower" and store.code_tower = 0
{
reward = 250
store.code_tower = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "arrow" and store.code_arrow = 0
{
reward = 250
store.code_arrow = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "happy" and store.code_happy = 0
{
reward = 250
store.code_happy = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "cookie" and store.code_cookie = 0
{
reward = 200
store.code_cookie = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "card" and store.code_card = 0
{
store.code_card = 1
room=rm_open_card
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded a Card!"
save_game()
}
if myResult = "ironpost" and store.code_ironpost = 0
{
reward = 200
store.code_ironpost = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "lonetower" and store.code_lonetower = 0
{
reward = 300
store.code_lonetower = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "hundo" and store.code_hundo = 0
{
reward = 100
store.code_hundo = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "levelup" and store.code_levelup = 0
{
reward = 300
store.code_levelup = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "snack" and store.code_snack = 0
{
reward = 100
store.code_snack = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "bonfire" and store.code_bonfire = 0
{
reward = 250
store.code_bonfire = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "zombie" and store.code_zombie = 0
{
reward = 300
store.code_zombie = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "taco" and store.code_taco = 0
{
reward = 200
store.code_taco = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "funtimes" and store.code_funtimes = 0
{
reward = 200
store.code_funtimes = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "lordofflies" and store.code_lordofflies = 0
{
reward = 250
store.code_lordofflies = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "keylock" and store.code_keylock = 0
{
reward = 20
store.code_keylock = 1
store.keys += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Keys!"
save_game()
}
if myResult = "dig" and store.code_dig = 0
{
reward = 200
store.code_dig = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "onion" and store.code_onion = 0
{
reward = 200
store.code_onion = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}
if myResult = "phoenix" and store.code_phoenix = 0
{
reward = 250
store.code_phoenix = 1
store.gems += reward
instance_create_depth(x,y-8,depth-1000,show_reward).myReward = "Rewarded "+string(reward)+" Gems!"
save_game()
}